// BTEC IT Unit 2 — Cyber Security & Incident Management App
(function() {
'use strict';
...
const SESSION_TYPES = [
  ...
  'quiz_it_u2', 'practice_it_u2', 'mock_it_u2', 'flashcard_it_u2',
  'quiz_it_aaq_u2', 'practice_it_aaq_u2', 'mock_it_aaq_u2', 'flashcard_it_aaq_u2'
];
...
function addPracticeProgress(count) {
  ...
  saveDailyTasksState(state);
  if (document.getElementById('view-dashboard')?.classList.contains('active')) renderYou();
}

function markFlashReviewStarted() {
  const changed = completeDailyTask('flashReview');
  if (changed && document.getElementById('view-dashboard')?.classList.contains('active')) renderYou();
}
...
async function renderYou() {
  if (document.getElementById('view-dashboard')?.classList.contains('active') === false) return;
...
function ensureProgressSidebar() {
  if (document.getElementById('u2-progress-sidebar')) return;
  let toggle = null;
  const sidebar = el('aside', { id: 'u2-progress-sidebar', class: 'u2-progress-sidebar hidden' },

    el('div', { class: 'u2-sb-head' },
      el('span', { class: 'u2-sb-chip' }, '⚡ Progress Boost'),
      el('button', { class: 'u2-sb-close', onclick: () => {
        window._u2SidebarDismissed = true;
        sidebar.classList.remove('open');
        sidebar.classList.add('hidden');
        document.body.classList.remove('has-u2-sidebar');
        if (toggle) toggle.style.display = '';
      } }, '✕')
    ),
    el('div', { id: 'u2-sb-body', class: 'u2-sb-body' },
      el('p', { class: 'muted' }, 'Loading progress...')
    )
  );
  toggle = el('button', {
    id: 'u2-sidebar-toggle',
    class: 'u2-sidebar-toggle',
    onclick: () => {
      window._u2SidebarDismissed = false;
      sidebar.classList.remove('hidden');
      document.body.classList.add('has-u2-sidebar');
...
async function refreshProgressSidebar(force) {
  ...
  const sidebar = document.getElementById('u2-progress-sidebar');
  const toggle = document.getElementById('u2-sidebar-toggle');
  const body = document.getElementById('u2-sb-body');
  ...
  try {
    ...
    body.innerHTML = `
      ...
      <div class="u2-sb-progress"><span style="width:${pct}%;"></span></div>
      <div class="u2-sb-note">${milestone - xpWeek} XP to next milestone (${milestone})</div>

      <div class="u2-sb-kpi streak">
        <div class="kpi-emoji">🔥</div>
        <div>
      ...
      <div class="u2-sb-mini-title">This week's top 3</div>
      <div class="u2-sb-mini-list">
        ${(leaders || []).length ? (leaders || []).map((u, i) => `
          <div class="u2-sb-mini-row">
            <span>${medals[i]} ${u.username ? '@' + u.username : (u.full_name || 'Anonymous')}</span>
            <strong>${u.xp_this_week} XP</strong>
          </div>
        `).join('') : '<div class="u2-sb-empty">No XP yet this week.</div>'}
      </div>

      <button class="btn primary u2-sb-btn" id="u2-sb-view-progress">View full progress</button>
    `;
    document.getElementById('u2-sb-view-progress')?.addEventListener('click', () => switchTab('progress'));
  } catch (e) {
...
function switchTab(name) {
  const parentTab = ({ mock: 'questions', practice: 'questions', browse: 'questions', quiz: 'revise', flash: 'revise', guide: 'revise', spec: 'revise', papers: 'papers', progress: 'progress', dashboard: 'dashboard' })[name] || name;
  $$('.tab').forEach(t => t.classList.toggle('active', t.dataset.tab === parentTab));
  $$('.view').forEach(v => v.classList.toggle('active', v.id === 'view-' + name));
  if (name === 'guide') renderRevisionGuide();
  if (name === 'progress') renderProgress();
  if (name === 'dashboard') renderYou();
...
const GUIDE_FULL_UNLOCK_EXPIRY_KEY = 'ra10_guide_full_unlock_it_u2_expires_at';
...
function renderPracticeControls() {
  ...
  if (!aimSel.dataset.ready) {
    ['A','B','C','D'].forEach(a => aimSel.appendChild(el('option', { value: a }, `Aim ${a}`)));
...
function extractSchemeKeywords(point) {
  const p = String(point || '').replace(/\([A-F]\d+\)/g, '').trim();
...
function startWeakPractice(aims) {
  ...
  window._practiceSession = {
    aims: aims && aims.length ? aims : ['A','B','C','D'],
    total: 0,
    correct: 0,
    aimBreakdown: {},
    attemptedIds: new Set(),
