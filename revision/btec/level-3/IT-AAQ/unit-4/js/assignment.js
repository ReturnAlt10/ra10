...
(function () {
  ...
  var CHECKLIST_KEY = 'ra10_u4_checklist';
  ...
  function taskIcon(code) {
    return code === 'task2' ? icon(0x1F5C2) + icon(0xFE0F) : code === 'task3' ? icon(0x1F6E0) + icon(0xFE0F) : icon(0x1F50D);
  }
  function taskShort(code) {
    return code === 'task1' ? 'Research + scoping' : code === 'task2' ? 'Design the database' : 'Build + test the database';
  }
  function taskVerb(code) {
    return code === 'task1' ? 'Start scoping' : code === 'task2' ? 'Start designing' : 'Start building';
  }

  function structureTip(code) {
    if (code === 'task1') {
      return 'Write a scoping document with headings: 1) Purpose (the problem the database solves + technical requirements), 2) Research into how similar databases are structured (tables and keys), 3) A normalisation walkthrough (UNF \u2192 3NF) of a sample table, 4) The proposed logical structure (tables, fields, keys, relationships) using accurate technical vocabulary.';
    }
    if (code === 'task2') {
      return 'Produce, in order: 1) a conceptual ERD (entities + relationships), 2) a logical ERD (add attributes, primary keys, foreign keys), 3) a data dictionary (every table, field name, data type, length, validation), 4) form/menu/query/report designs, 5) a test plan and implementation timescale. Then self-review and show your refinements.';
    }
    return 'Build the database from your design, then document: 1) creating tables, relationships and validation (screenshots + SQL), 2) forms, queries and reports you built, 3) a test plan using normal, erroneous and extreme data (expected vs actual), 4) usability testing with real users, 5) a self-review with the optimisations and refinements you made.';
  }
  ...
  function render() {
    ...
    html += '<div class="assign-samples">' +
      ...
      '<p>Download a full Pearson-style assignment brief to work on, plus a matching <b>.accdb</b> database with real sample data \u2014 open it in Microsoft Access to see how tables, relationships and queries are built.</p></div>' +
      ...
      (briefs.length ? briefs.map(function (b, i) {
        return '<div class="assign-sample-card">' +
          ...
          '</div>' +
          '<div class="as-pdf-samples">' +
            '<a class="btn assign-accdb-dl" href="samples/' + esc(b.accdb || '') + '" download>' + icon(0x1F5C4) + icon(0xFE0F) + ' Download .accdb</a>' +
            '<span class="as-sample-note">' + icon(0x1F4CA) + ' sample database included</span>' +
          '</div>' +
          '</div>';
    ...
    html += '<div class="assign-cta">' +
      '<button class="btn primary big assign-go" data-goto="' + (activeTask === 'task2' ? 'access' : activeTask === 'task3' ? 'sql-lab' : 'norm-lab') + '">' +
        taskVerb(activeTask) +
        '<small>' + (activeTask === 'task2' ? 'open the Access tutorials' : activeTask === 'task3' ? 'open the SQL Lab' : 'open the Normalisation Lab') + '</small></button>' +
      ...
      '<div class="asg-grid">' +
        ...
        '<div class="asg-card asg-distinction">' +
          ...
          '<ul>' +
            ...
            '<li><b>Use accurate technical vocabulary</b> \u2014 e.g. \u201Crelation\u201D, \u201Cattribute\u201D, \u201Cprimary key\u201D, \u201Cnormalisation\u201D, \u201Creferential integrity\u201D.</li>' +
        ...
        '<div class="asg-card asg-mistakes">' +
          ...
          '<ul>' +
            '<li>No normalisation evidence (you need UNF \u2192 3NF for A.M1 / A.D1).</li>' +
            '<li>No test plan with <b>expected vs actual</b> results \u2014 and no erroneous/extreme data.</li>' +
            '<li>A data dictionary that\u2019s missing data types or validation rules.</li>' +
    ...
    if (cl) cl.querySelectorAll('li').forEach(function (li) {
      li.querySelector('input').addEventListener('change', function () {
        ...
        try {
          ...
          localStorage.setItem('ra10_u4_assignment_tasks', JSON.stringify(doneTasks));
        } catch (e) {}
      });
    });
  ...
  window._openBrief = function (brief, taskCode) {
    ...
    var dest = t === 'task2' ? 'access' : t === 'task3' ? 'sql-lab' : 'norm-lab';
    var destLabel = t === 'task2' ? 'Open the Access tutorials' : t === 'task3' ? 'Open the SQL Lab' : 'Open the Normalisation Lab';
    ...
    modal.addEventListener('click', function (e) {
      ...
      if (e.target.closest('[data-bdict]')) { try { localStorage.setItem('ra10_u4_last_brief', JSON.stringify({ title: brief.title, audience: brief.audience, at: Date.now() })); } catch (e2) {} close(); return; }
  ...
  window._openAssignmentPdf = function (briefIndex) {
    ...
    function scenarioFor(code) {
      var map = {
        task1: 'Examine the principles of relational database models, data storage and normalisation for the given client brief. Research existing database solutions and how they structure data, then produce a preliminary scoping document showing the purpose, technical requirements, a normalisation walkthrough and the proposed logical structure (tables, fields, keys and relationships) to meet the client\u2019s requirements.',
        task2: 'Use relational database design skills and techniques to design a database solution in response to the client brief. Produce a design specification including a conceptual and logical entity relationship diagram, a data dictionary, user interface designs (forms, menus, queries and reports), a test plan and an implementation plan with timescales. Review and refine your designs against the brief.',
        task3: 'Develop a relational database solution in response to the client brief. Use an appropriate DBMS and SQL to create tables, relationships and validation, then build the user interface and generate queries and reports. Populate the database and conduct testing (normal, erroneous and extreme data) and a usability test, then review and optimise the solution.'
      };
      return map[code] || '';
    }

    function evidenceFor(code) {
      if (code === 'task1') return ['A preliminary scoping document.', 'Evidence of research into existing databases.', 'A normalisation walkthrough (UNF to 3NF).', 'A proposed logical structure with tables and keys.'];
      if (code === 'task2') return ['A design specification.', 'Conceptual and logical entity relationship diagrams.', 'A data dictionary.', 'User interface designs (forms, menus, queries, reports).', 'A test plan and implementation plan.'];
      return ['The developed relational database solution.', 'Evidence of tables, relationships and validation.', 'Queries and reports produced.', 'A test plan and test results (normal, erroneous and extreme data).', 'A usability test and a self-review with refinements.'];
    }
    ...
    var contextTitle = brief.title || 'College Course Enrolment Database';
    var contextBlurb = brief.scenario || 'Produce a relational database solution. Replace a spreadsheet-based system with a database that manages learners, courses, subjects and staff, records enrolments, and produces useful reports on course enrolment and learner progress.';
    var mustIncludes = (Array.isArray(brief.mustIncludes) && brief.mustIncludes.length)
      ? brief.mustIncludes
      : ['tables for Learner, Course, Subject, Staff and Enrolment', 'appropriate primary and foreign keys with one-to-many relationships', 'data validation rules', 'an enrolment form with combo boxes', 'queries using multiple criteria and wildcards', 'a report grouping enrolments by course', 'task automation using a macro or button', 'a test plan with normal, erroneous and extreme data'];

    var pagesHtml = '';

    // Cover page — instructions + introductory context
    pagesHtml += '<div class="pv-page">' +
      '<div class="pv-masthead"><span class="pv-paper-code">W80503A</span>' +
      '<h1>Pearson BTEC Level 3 AAQ BTEC National in IT</h1>' +
      '<div class="pv-sub">Unit 4: Relational Database Development \u2014 Pearson Set Assignment Brief</div>' +
      '</div>' +
      ...
      '<p><strong>Produce a relational database solution for:</strong> ' + escp(contextTitle) + '.</p>' +
      '<p>' + escp(contextBlurb) + '</p>' +
      '<div class="pv-evid"><b>The solution must include:</b><ul>' +
      mustIncludes.map(function (mi) { return '<li>' + escp(mi) + '</li>'; }).join('') +
      '</ul></div>' +
      '<div class="pv-footer"><span>RA10 \u00B7 Unit 4 Assignment Brief</span><span class="pv-pagenum">Page 1</span></div>' +
      '</div>';
    ...
    tasks.forEach(function (task, idx) {
      ...
      pagesHtml += '<div class="pv-page">' +
        ...
        '</div>' +
        '<div class="pv-footer"><span>RA10 \u00B7 Unit 4 Assignment Brief</span><span class="pv-pagenum">Page ' + (2 + idx) + '</span></div>' +
        '</div>';
    });
    ...
    pagesHtml += '<div class="pv-page">' +
      ...
      '<p>This is a practice replica produced by RA10 for revision. It mirrors the format of the real Pearson Set Assignment Brief (W80503A) but is not an official Pearson document.</p>' +
      '<div class="pv-footer"><span>RA10 \u00B7 Unit 4 Assignment Brief</span><span class="pv-pagenum">Page ' + (2 + tasks.length) + '</span></div>' +
      '</div>';
  ...
  var WALKTHROUGHS = {
    task1: {
      summary: 'Research how relational databases store data, write down what the client needs, then normalise a sample table and produce a scoping document with the proposed tables, keys and relationships.',
      evidence: [
        'Client needs as a clean list (purpose, tech requirements)',
        'Research: how existing databases structure tables + keys',
        'A normalisation walkthrough (UNF \u2192 1NF \u2192 2NF \u2192 3NF)',
        'Proposed logical structure (tables, fields, keys, relationships)',
        'Accurate technical vocabulary throughout'
      ],
      steps: [
        '<b>1 \u00B7 Read the brief like a detective</b> \u2014 list every table, field and feature the client needs.',
        '<b>2 \u00B7 Research existing databases</b> \u2014 note how similar systems store data (tables and keys).',
        '<b>3 \u00B7 Normalise a sample</b> \u2014 walk a flat table through UNF \u2192 3NF, splitting out repeating groups and dependencies.',
        '<b>4 \u00B7 Propose the structure</b> \u2014 list your tables, primary keys, foreign keys and relationships.',
        '<b>5 \u00B7 Use the vocabulary</b> \u2014 relation, attribute, tuple, PK, FK, normalisation \u2014 to hit D.'
      ]
    },
    task2: {
      summary: 'Design the full database: conceptual and logical ERDs, a data dictionary, user interface designs and a test plan \u2014 then review and refine it.',
      evidence: [
        'Conceptual ERD (entities + relationships)',
        'Logical ERD (adds attributes, PKs, FKs)',
        'Data dictionary (tables, fields, types, validation)',
        'Form / menu / query / report designs',
        'Test plan + implementation timescale'
      ],
      steps: [
        '<b>1 \u00B7 Conceptual ERD</b> \u2014 boxes for each entity, lines for relationships, label the type (1:M etc.).',
        '<b>2 \u00B7 Logical ERD</b> \u2014 add attributes, primary keys and foreign keys to each entity.',
        '<b>3 \u00B7 Data dictionary</b> \u2014 table of every field with name, data type, length and validation.',
        '<b>4 \u00B7 UI designs</b> \u2014 sketch forms (combo boxes, validation), menus, queries and reports.',
        '<b>5 \u00B7 Review + refine</b> \u2014 self-review against the brief, then document your improvements.'
      ]
    },
    task3: {
      summary: 'Build the database from your design using SQL, add forms/queries/reports, then test it with normal, erroneous and extreme data and optimise it.',
      evidence: [
        'The working database (tables + relationships)',
        'SQL / screenshots of tables, relationships and validation',
        'Forms, queries and reports you built',
        'Test plan: normal, erroneous + extreme data (expected vs actual)',
        'Usability test + self-review with refinements'
      ],
      steps: [
        '<b>1 \u00B7 Create tables</b> \u2014 use SQL (CREATE TABLE) or Access Design View with correct data types.',
        '<b>2 \u00B7 Link them</b> \u2014 define one-to-many relationships and enforce referential integrity.',
        '<b>3 \u00B7 Add validation</b> \u2014 rules, input masks and required fields on every form.',
        '<b>4 \u00B7 Build outputs</b> \u2014 queries (with criteria/wildcards), a parameter query, and a grouped report.',
        '<b>5 \u00B7 Test + optimise</b> \u2014 test normal/erroneous/extreme data, gather usability feedback, then refine and optimise.'
      ]
    }
  };
})();
