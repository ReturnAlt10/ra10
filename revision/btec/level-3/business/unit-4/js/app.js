/* ============================================================
   BTEC Business Unit 4 — Managing an Event
...
(function () {
  ...
  const FLASH_STORE_KEY  = 'btec-bus-u4-flash-progress-v1';
  const SESSION_TYPES = [
    'quiz_business_u4', 'practice_business_u4', 'mock_business_u4', 'flashcard_business_u4',
    'quiz', 'practice', 'mock', 'flashcard'
  ];

  // ---------- Spec (Business Unit 4) ----------
  const SPEC = {
    A: {
      title: 'Role of an Event Organiser',
      short: 'Tasks, skills and skills audit',
      topics: [
        { code: 'A1', name: 'Tasks of an event organiser' },
        { code: 'A1.1', name: 'Planning, organising, budgeting, promoting, coordinating' },
        { code: 'A2', name: 'Skills of an event organiser' },
        { code: 'A2.1', name: 'Communication, organisation, problem solving, negotiation, leadership, teamwork, time management' },
        { code: 'A3', name: 'Skills audit' },
        { code: 'A3.1', name: 'Formats and purpose of a skills audit' }
      ]
    },
    B: {
      title: 'Feasibility of an Event',
      short: 'Types of event, factors affecting success, feasibility',
      topics: [
        { code: 'B1', name: 'Types of event and factors affecting success' },
        { code: 'B1.1', name: 'Business, social, sports, entertainment events' },
        { code: 'B2', name: 'Feasibility of an event' },
        { code: 'B2.1', name: 'Cost, resources, venue, timing; critical success factors' }
      ]
    },
    C: {
      title: 'Planning the Event',
      short: 'Planning tools and key factors',
      topics: [
        { code: 'C1', name: 'Planning tools' },
        { code: 'C1.1', name: 'Gantt chart, action plans, critical path analysis' },
        { code: 'C2', name: 'Factors to consider' },
        { code: 'C2.1', name: 'Budget, resources, health & safety, risk assessment, contingency' }
      ]
    },
    D: {
      title: 'Staging and Managing the Event',
      short: 'Managing on the day and problem solving',
      topics: [
        { code: 'D1', name: 'Managing the event' },
        { code: 'D1.1', name: 'Coordinating staff, schedule, safety, budget' },
        { code: 'D2', name: 'Problem solving' },
        { code: 'D2.1', name: 'Dealing with unexpected problems' }
      ]
    },
    E: {
      title: 'Evaluation and Reflection',
      short: 'Evaluating the event and skills development',
      topics: [
        { code: 'E1', name: 'Evaluation of the event' },
        { code: 'E1.1', name: 'Feedback, reviewing against objectives, budget' },
        { code: 'E2', name: 'Review of personal skills' },
        { code: 'E2.1', name: 'Reflection and skills development' }
      ]
    }
  };
  ...
  async function saveSession(sessionType, data) {
    ...
    try {
      ...
      if (!payload.total && sessionType !== 'flashcard_business_u4') return;
  ...
  const GUIDE_FULL_UNLOCK_EXPIRY_KEY = 'ra10_guide_full_unlock_business_u4_expires_at';
  ...
  function renderMock(mock) {
    ...
    wrap.appendChild(el('div', { class: 'paper-header' },
      el('h3', null, 'BTEC Level 3 Business — Unit 4: Managing an Event'),
  ...
  async function endPracticeSession() {
    ...
    await saveSession('practice_business_u4', {
  ...
  async function renderProgress() {
    ...
    const typeLabel = {
      'quiz_business_u4': '🧠 Quiz',
      'practice_business_u4': '✍️ Practice',
      'mock_business_u4': '📄 Mock Paper',
      'flashcard_business_u4': '🃏 Flashcards',
  ...
  async function endFlashSession() {
    ...
    await saveSession('flashcard_business_u4', {
