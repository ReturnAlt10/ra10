/* ============================================================
   BTEC Business Unit 1 — Exploring Business
...
(function () {
  ...
  const FLASH_STORE_KEY  = 'btec-bus-u1-flash-progress-v1';
  const SESSION_TYPES = [
    'quiz_business_u1', 'practice_business_u1', 'mock_business_u1', 'flashcard_business_u1',
    'quiz', 'practice', 'mock', 'flashcard'
  ];

  // ---------- Spec (Business Unit 1) ----------
  const SPEC = {
    A: {
      title: 'Features of Businesses',
      short: 'Ownership, liability, sectors, size, stakeholders, success',
      topics: [
        { code: 'A1', name: 'Features of businesses: ownership, liability, purpose, sectors, scope, size' },
        { code: 'A1.1', name: 'Private, public and not-for-profit ownership' },
        { code: 'A1.2', name: 'Limited vs unlimited liability' },
        { code: 'A1.3', name: 'Sectors: primary, secondary, tertiary, quaternary' },
        { code: 'A1.4', name: 'Scope (local, national, international) and size (micro to large)' },
        { code: 'A2', name: 'Stakeholders and their influence' },
        { code: 'A2.1', name: 'Internal and external stakeholders' },
        { code: 'A2.2', name: 'How stakeholders influence business success' },
        { code: 'A3', name: 'Effective business communications' },
        { code: 'A3.1', name: 'Written and oral communication methods' },
        { code: 'A3.2', name: 'Importance of communication to success' }
      ]
    },
    B: {
      title: 'How Businesses Are Organised',
      short: 'Organisational structure, functional areas, aims and objectives',
      topics: [
        { code: 'B1', name: 'Structure and organisation' },
        { code: 'B1.1', name: 'Hierarchical, flat, matrix and holacratic structures' },
        { code: 'B1.2', name: 'Functional/operational areas' },
        { code: 'B2', name: 'Aims and objectives' },
        { code: 'B2.1', name: 'Aims of private, public and not-for-profit businesses' },
        { code: 'B2.2', name: 'SMART objectives' }
      ]
    },
    C: {
      title: 'The Business Environment',
      short: 'External, internal and competitive environment; situational analysis',
      topics: [
        { code: 'C1', name: 'External environment (PESTLE)' },
        { code: 'C1.1', name: 'Political, economic, social, technological, legal, environmental factors' },
        { code: 'C2', name: 'Internal environment' },
        { code: 'C2.1', name: 'Corporate culture, CSR, ethics' },
        { code: 'C3', name: 'Competitive environment' },
        { code: 'C3.1', name: 'Competition and competitive advantage' },
        { code: 'C4', name: 'Situational analysis' },
        { code: 'C4.1', name: 'PESTLE, SWOT, 5Cs, Porter\u2019s Five Forces' }
      ]
    },
    D: {
      title: 'Business Markets',
      short: 'Market structures, demand, supply, price, elasticity',
      topics: [
        { code: 'D1', name: 'Different market structures' },
        { code: 'D1.1', name: 'Perfect and imperfect competition' },
        { code: 'D2', name: 'Demand, supply and price' },
        { code: 'D2.1', name: 'Influences on demand and supply; price elasticity' },
        { code: 'D3', name: 'Pricing and output decisions' },
        { code: 'D3.1', name: 'Responses to competitors in different market structures' }
      ]
    },
    E: {
      title: 'Innovation and Enterprise',
      short: 'Role, benefits and risks of innovation and enterprise',
      topics: [
        { code: 'E1', name: 'Role of innovation and enterprise' },
        { code: 'E1.1', name: 'Innovation, enterprise and creative thinking' },
        { code: 'E2', name: 'Benefits and risks of innovation' },
        { code: 'E2.1', name: 'Benefits (growth, USP) and risks (resistance, ROI failure)' }
      ]
    }
  };

  const AIM_LETTERS = Object.keys(SPEC);
  ...
  async function saveSession(sessionType, data) {
    ...
    try {
      ...
      if (!payload.total && sessionType !== 'flashcard_business_u1') return;
  ...
  const GUIDE_FULL_UNLOCK_EXPIRY_KEY = 'ra10_guide_full_unlock_business_u1_expires_at';
  ...
  function renderGuideAccessGate(container, aimASection) {
    ...
    card.innerHTML = timedUnlockActive
      ...
      `
      : `
        <h4 style="margin-top:0">Aims B-E are locked</h4>
  ...
  function renderMockControls() {
    ...
    aimsRow.innerHTML = '';
    const selected = new Set(AIM_LETTERS);
    AIM_LETTERS.forEach(a => {
  ...
  function renderMock(mock) {
    ...
    wrap.appendChild(el('div', { class: 'paper-header' },
      el('h3', null, 'BTEC Level 3 Business — Unit 1: Exploring Business'),
  ...
  function renderPracticeControls() {
    ...
    if (!aimSel.dataset.ready) {
      AIM_LETTERS.forEach(a => aimSel.appendChild(el('option', { value: a }, `Aim ${a}`)));
  ...
  async function endPracticeSession() {
    ...
    await saveSession('practice_business_u1', {
  ...
  async function renderProgress() {
    ...
    const typeLabel = {
      'quiz_business_u1': '🧠 Quiz',
      'practice_business_u1': '✍️ Practice',
      'mock_business_u1': '📄 Mock Paper',
      'flashcard_business_u1': '🃏 Flashcards',
  ...
  function startWeakPractice(aims) {
    ...
    window._practiceSession = { aims: aims && aims.length ? aims : AIM_LETTERS, total: 0, correct: 0, aimBreakdown: {}, attemptedIds: new Set(), scoreByQuestion: {} };
    practiceQueue = shuffle(pool);
    practiceIdx = 0;
    renderPracticeCard();
  }
  ...
  function startFlash() {
    ...
    window._flashSession = { aims: aim ? [aim] : AIM_LETTERS, total: 0, correct: 0, aimBreakdown: {}, attemptedIds: new Set(), knownIds: new Set() };
  ...
  async function endFlashSession() {
    ...
    await saveSession('flashcard_business_u1', {
